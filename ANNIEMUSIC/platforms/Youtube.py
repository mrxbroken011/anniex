...
async def get_telegram_file(telegram_url: str, video_id: str, file_type: str) -> str:
    ...
    try:
        ...
        if len(parts) < 2:
            logger.error(f"❌ ɪɴᴠᴀʟɪᴅ ᴛᴇʟᴇɢʀᴀᴍ ʟɪɴᴋ ғᴏʀᴍᴀᴛ: {telegram_url}")
            return None
        ...
        if os.path.exists(file_path):
            logger.info(f"✅ ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ: {video_id}")
            return file_path
        else:
            logger.error(f"❌ ᴛɪᴍᴇᴏᴜᴛ ᴛʀʏ ᴀɢᴀɪɴ: {video_id}")
            return None
        
    except Exception as e:
        logger.error(f"❌ ғᴀɪʟᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ: {video_id}: {e}")
        return None
...
async def download_song(link: str) -> str:
    ...
    logger = LOGGER("ʙʀᴏᴋᴇɴ x ʏᴛ ᴀᴘɪ")
    logger.info(f"🎵 sᴛᴀʀᴛɪɴɢ ᴅᴏᴡɴʟᴏᴀᴅ ғᴏʀ: {video_id}")

    if not video_id or len(video_id) < 3:
        logger.error(f"❌ɪɴᴠᴀʟɪᴅ ᴠɪᴅᴇᴏ ɪᴅ: {video_id}")
        return None
    ...
    if os.path.exists(file_path):
        logger.info(f"🎵ғɪʟᴇ ᴇxɪsᴛs: {video_id}")
        return file_path
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            logger.info(f"🎧 ʀᴇǫᴜᴇsᴛɪɴɢ ғʀᴏᴍ ᴀᴘɪ: {video_id}")
            ...
            async with session.get(
                ...
                if response.status != 200:
                    logger.error(f"❌ ᴀᴘɪ ᴇʀʀᴏʀ: {response.status}")
                    return None
                ...
                if data.get("telegram_url"):
                    ...
                    if downloaded_file:
                        logger.info(f"🎉 sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ: {video_id}")
                        return downloaded_file
                    else:
                        logger.error(f"⚠️ ᴅᴏᴡɴʟᴏᴀᴅ ғᴀɪʟᴇᴅ ғᴏʀ: {video_id}")
                        return None
                else:
                    logger.error(f"❌ ɴᴏ ʀᴇsᴘᴏɴsᴇ ғʀᴏᴍ sᴇʀᴠᴇʀ")
                    return None

    except asyncio.TimeoutError:
        logger.error(f"⏱️ ᴛɪᴍᴇᴏᴜᴛ: {video_id}")
        return None
    except Exception as e:
        logger.error(f"❌ ғᴀɪʟᴇᴅ ᴛᴏ ʀᴇᴀᴄʜ ᴀᴘɪ {video_id}")
        return None
